...
from src.load.ContractLoad import ContractLoad
from src.load.TransactionLoad import TransactionLoad
...
class CsvPipe(object):
    """Class with steps pipe CSV"""
    ...
    def __loadStep(self):
        self.df_contract = ContractLoad(self.spark_session).load().withColumn("dt_partition", F.lit(self.args.date))
        self.df_transaction = TransactionLoad(self.spark_session).load().withColumn("dt_partition", F.lit(self.args.date))
